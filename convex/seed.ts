...
export const clearData = mutation({
  ...
  handler: async (ctx) => {
    ...
    const [candidates, jobs, interviews, timeline, assessments, notes, applications] = await Promise.all([
...
export const seedData = mutation({
  ...
  handler: async (ctx) => {
    ...
    const jobIds = await Promise.all(
      jobs.map(job => ctx.db.insert("jobs", job))
    );
    ...
    await ctx.db.insert("notes", {
      ...
      role: "Recruiter",
      date: "2024-01-18",
      content: "Strong candidate with excellent technical skills. Previous experience at FAANG companies.",
    });

    ...
    const funnelData = [
      { stage: 'Applied', count: candidates.length, color: '#3b82f6' },
      {
        ...
        count: candidates.filter((c) =>
          ['screening', 'interviewed', 'offered', 'hired'].includes(c.status)
        ).length,
        color: '#8b5cf6',
      },
      {
        stage: 'Interview',
        count: candidates.filter((c) => ['interviewed', 'offered', 'hired'].includes(c.status))
          .length,
        color: '#06b6d4',
      },
      {
...
export const getSourceEffectiveness = query({
  handler: async (ctx) => {
    ...
    const sourceData = candidates.reduce(
      (acc, candidate) => {
    ...
    const sourceEffectiveness = Object.values(sourceData)
      .map((source: any) => ({
        ...source,
        hireRate:
  ...
  handler: async (ctx) => {
    ...
    const avgRating =
      interviewsWithFeedback.length > 0
        ? (
            interviewsWithFeedback.reduce((sum, i) => sum + (i.rating || 0), 0) /
